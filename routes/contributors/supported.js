...
const contributors = {
    method: 'GET',
    path: '/radar/contributors/{name}/supported',
    config: {
        pre: [
            [{
        ...
        handler: (request, reply) => {
            ...
            if (Accept.parseAll(request.headers).mediaTypes.includes('text/html')) {
                ...
                reply.view('contributors/supported', {
                    title: `${request.params.name} - Supported`,
        ...
        description: 'Retrieve support info related to the contributor',
        notes: 'Retreives all blips that a contributor has supported (rated 0 - 5)',
        plugins: {
            'hapi-swagger': {
                responses: {
                    'default': {
