    ...
    config: {
        ...
        handler: (request, reply) => {
            ...
            if (Accept.parseAll(request.headers).mediaTypes.includes('text/html')) {
                ...
                reply.view(`blips/proposed`, _.merge(request.pre.features, {
                    title: `Blip - Proposed`,
        ...
        description: 'Retrieve the contributor that proposed this blip (created it)',
        notes: 'Returns a contributor object',
        plugins: {
            'hapi-swagger': {
                responses: {
                    'default': {
